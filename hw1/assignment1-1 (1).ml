...
h::[]->h::[]
|
 (h::m::t)-> if h = m then remove_stutter (m::t)
 else h :: remove_stutter(m::t);;
...
|[]->b
| h::t ->
if elem h b then h::union t (remove h b)
...
let rec additivePersistence n =
  if n<=9 then 0
   else let rec count n=
      if n<=0 then 0
      else((n mod 10) + (count(n/10))) in
         if (count(n))>=10 then 1+additivePersistence(count(n)) 
         else 1 ;;


let rec digitalRoot n =
  if n<=0 then 0
  else let sum = ((n mod 10) + (digitalRoot (n/10))) in
       if sum>=10 then digitalRoot(sum)
       else sum;;
